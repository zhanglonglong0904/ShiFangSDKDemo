...
//writable -- 如果为true，允许写访问权限;如果为false，写访问权限是不允许的。
    ...
    public static boolean write2File(Context context, String encrypt, String fileStr) {
        if(TextUtils.isEmpty(encrypt)){
            Log.i("write2File", " write return for empty");
            return false;
        }
        ...
        boolean fileFromString = writeFileFromString(file, encrypt, false);
        if(fileFromString){
            result = true;
            Log.i("write2File", " write success");
        }else{
            result = false;
            Log.i("write2File", " write faile");
        }
        return result;
    }
